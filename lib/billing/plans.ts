/**
 * Paid plan catalog. Stripe Price IDs come from env so the same code works
 * across test/live and lets you change pricing in Stripe without a deploy.
 *
 * Set these in your environment after creating the recurring Prices in Stripe:
...
export const PLANS: Plan[] = [
  {
    ...
    name: "Nula Pro",
    interval: "month",
    priceId: process.env.STRIPE_PRICE_PRO_MONTHLY?.trim() ?? "",
    amount: 2900,
    currency: "usd",
...
/** Plans that have a Stripe Price ID configured (i.e. are purchasable). */
